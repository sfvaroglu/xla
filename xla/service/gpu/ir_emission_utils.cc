...
// Returns true if `maybe_variable` is marked as a dynamic variable.
bool IsDynamicVariable(const HloInstruction* maybe_variable,
                       const VerifiedLoop& loop) {
  if (!IsLoopCarriedVariable(maybe_variable, loop)) {
    return false;
  }
  ...
  int64_t tuple_idx = maybe_variable->tuple_index();
...
std::optional<const HloInstruction*> VerifyInductionVariable(
  ...
  for (const HloInstruction* gte : dependencies.get_tuple_elements) {
    if (IsInductionVariable(gte, loop)) {
      ...
      // Dynamic variables are also acceptable because they represent tuple indices
      // used in DS/DUS that can be optimized by FusionDynamicMemcpyRewriter.
      if (induction_var) {
        // This should never happen.
