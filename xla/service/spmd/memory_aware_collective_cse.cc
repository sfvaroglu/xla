...
constexpr int64_t kVeryCloseDistance = 10;
constexpr int64_t kTinyBufferBytes = 1 << 20;
constexpr int64_t kMediumBufferBytes = 512 << 20;
...
bool ShouldCSECollectives(const HloInstruction* earlier_coll,
  ...
  if (buffer_size < kMediumBufferBytes && distance <= kMediumDistance) {
    VLOG(2) << "CSE approved: medium buffer (" << buffer_size << " bytes) "
...
absl::StatusOr<bool> MemoryAwareCollectiveCSE::RunImpl(
    HloModule* module,
  ...
  for (auto comp : module->computations(execution_threads)) {
    ...
    TF_ASSIGN_OR_RETURN(auto comp_changed, RunOnComputation(comp));
